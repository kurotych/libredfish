// Read-only: runs Redfish::get_managers against a real BMC and prints
// the list of Manager ids. Reads BMC, USER, PASS from env.
//
// Run with:
//   RUST_LOG=debug BMC=https://10.213.2.184 USER=admin PASS=... \
//       cargo run --example get_managers
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("found {} manager(s)", managers.len());
    for id in &managers {
        println!("- {id}");
    }
